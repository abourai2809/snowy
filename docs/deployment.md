...
`SUPABASE_SERVICE_ROLE_KEY` is used only by Vercel serverless functions, currently staff signup and Admin staff password reset. Never expose it with a `VITE_` prefix and never commit it to Git.
