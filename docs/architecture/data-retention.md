...
| `attendance_selfie_checks` | Supabase forever index + Drive image archive | Image preview in Supabase Storage for 3 days; check rows remain | Raw selfie image file in Drive, referenced by file id | Full AI verdict row, archive provider, archive path/file id, and storage deletion timestamp remain. |
